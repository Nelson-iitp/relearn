""" SPG - Simple Policy Gradients


    ~ implements policy gradients with 2 types of advantage estimate ('r2g' argument)
        ~ Trajectory Return
        ~ Rewards-2-Go 

    ~ NOTE: does not use Value function for advantage
...
def train(
    ...
    for epoch in range(epochs):
        ...
        for _ in range(batch_size):
            ...
            if r2g: 
                trajectory.update({'W': reversed_cumulative_sum(trajectory['dR']) }) # advantage - uses reward-2-go
            else:
                trajectory.update({'W': np.zeros(n) + trajectory['dRET'] })    # advantage - uses episode-return 
        ...
        batch_W = tt.tensor(np.hstack( tuple( [D[i]['W']    for i in range(batch_size)] ) ), dtype=dtype, device=device)
