...
class PIE:
    ...
    def __init__(self, state_dim, LL, action_dim, opt, cost, lr, double=False, tuf=0,  device='cpu', dtype=T.float32, relu_act=True): 
    ...
    def _prepare_batch(self, memory, batch_size):
        ...
        for i in batch_size:
            ...
            #T.arange(batch_size).to(dtype=T.long), \
        """
        return  batch_size, \
    ...
    def learn(self, memory, batch, lr=0.0, dis=1):
        ...
        indices = T.arange(0, steps, 1, dtype=T.long)
        ...
        if not self.double:
            ...
            updater[indices] = target_val[indices, (target_next[indices]).to(dtype=T.long)]
        updated_q_values = reward + dis * updater * (1 - done)
        
        # Compute prediction and loss
        pred = self.Q(cS)
        target = pred.detach().clone()
        target[indices, (act[indices]).to(dtype=T.long)] = updated_q_values[indices]
        loss =  self.loss_fn(pred, target)  #T.tensor()
        self.optimizer.zero_grad()
